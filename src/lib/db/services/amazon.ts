...
import type { AmazonOrder, LegacyAmazonOrder, LegacyOrderItem } from '$lib/types';
...
export class AmazonService {
	...
	async syncOrders(
		...
		try {
			...
			if (!(await this.isApiConfigured())) {
				throw new Error(
					'Amazon API is not properly configured. Please set up your API credentials first.'
				);
			}
	...
	private async processReviewRequest(
		...
		try {
			...
			if (result.success) {
				...
				await this.markOrderAsSkipped(
					order.id,
					result.error ?? 'No solicitation actions available'
				);
	 ...
	 */
	async retryFailedReviewRequests(): Promise<{
		success: boolean;
		retried: number;
		successCount: number;
	}> {
		try {
			...
			const retryableRequests = failedRequests.filter(
				(req) => req.status === ReviewRequestStatus.FAILED && req.retryCount < 3
			);
		...
		try {
			...
			for (const order of eligibleOrders) {
				try {
					...
					await this.handleReviewRequestFailure(
						order,
	...
	private async sendReviewRequest(
		order: LegacyAmazonOrder
	): Promise<{ success: boolean; error?: string; notEligible?: boolean }> {
		try {
			...
			if ('notEligible' in result && result.notEligible) {
				console.log(
					`Order ${order.amazonOrderId} is not eligible for review requests: ${result.reason}`
				);
