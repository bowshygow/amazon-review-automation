...
export class AmazonSPAPI {
  ...
  async getOrders(createdAfter: string, nextToken?: string): Promise<OrdersResponse> {
    const params: any = {
      operation: 'getOrders',
      endpoint: 'orders',
      query: {
  ...
  async getSolicitationActions(orderId: string): Promise<SolicitationResponse> {
    const response = await this.client.callAPI({
  ...
  async createReviewSolicitation(orderId: string): Promise<any> {
    const response = await this.client.callAPI({
